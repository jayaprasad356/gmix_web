...
@section('title', 'Chat Points Management')
@section('content-header', 'Chat Points Management')
@section('content-actions')
    <a href="{{route('chat_points.create')}}" class="btn btn-success"><i class="fas fa-plus"></i> Add New Chat Points</a>
@endsection
@section('css')
...
<div class="card">
    ...
    <div class="col-md-4 text-right">
        <!-- Search Form -->
        <form action="{{ route('chat_points.index') }}" method="GET">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <a href="{{ route('chat_points.edit', $chat_point) }}" class="btn btn-primary"><i class="fas fa-edit"></i></a>
                            <button class="btn btn-danger btn-delete" data-url="{{route('chat_points.destroy', $chat_point)}}"><i class="fas fa-trash"></i></button>
                        </td>
                        <td>{{$chat_point->id}}</td> 
                        <td>{{ optional($chat_point->user)->name }}</td> <!-- Display user name safely -->
                        <td>{{ optional($chat_point->chatUser)->name }}</td> <!-- Display the name of the chat user -->
        ...
        $(document).ready(function () {
            ...
            $('#user-filter').change(function () {
                if ($(this).val() !== '') {
                    ...
                    window.location.href = "{{ route('chat_points.index') }}";
                }
            });
            ...
            $(document).on('click', '.btn-delete', function () {
                ...
                swalWithBootstrapButtons.fire({
                    title: 'Are you sure?',
                    text: "Do you really want to delete this chat point?",
                    icon: 'warning',
